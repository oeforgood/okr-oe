...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const unsub=onSnapshot(ref,(snap)=>{
      ...
      setLoaded(true);
    },(e)=>{console.error("Firestore error",e);setLoaded(true);});
    return()=>unsub();
  },[]);

  const allSeasonsRef=useRef(allSeasons);
  useEffect(()=>{allSeasonsRef.current=allSeasons;},[allSeasons]);
  const seasonKeyRef=useRef(seasonKey);
  useEffect(()=>{seasonKeyRef.current=seasonKey;},[seasonKey]);

  function persist(aS){
    const ref=doc(db,"okr","data");
    setDoc(ref,{allSeasons:aS,seasonKey:seasonKeyRef.current,collObj:collObj,collSobj:collSobj})
  ...
  function toggleObj(id){const co={...collObj,[id]:!collObj[id]};setCollObj(co);persist(allSeasonsRef.current)}
  function toggleSobj(id){const cs={...collSobj,[id]:!collSobj[id]};setCollSobj(cs);persist(allSeasonsRef.current)}
