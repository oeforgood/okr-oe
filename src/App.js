...
function SettingsPage({onBack,currentUser,teamMembers,onSaveMembers,questions,onSaveQuestions}){
  ...
  return <div style={{minHeight:"100vh",background:"#f5f3ef",fontFamily:"system-ui,sans-serif"}}>
    ...
    <div style={{maxWidth:800,margin:"0 auto",padding:"24px 16px 60px"}}>
      ...
      {tab==="questions"&&<div style={{background:"#fff",borderRadius:10,border:"1px solid #e2ddd6",padding:"18px 20px"}}>
        <div style={{fontSize:13,fontWeight:600,marginBottom:14}}>Questions de l'Update hebdomadaire</div>
        {qs.map((q,i)=><div key={q.id} style={{marginBottom:14,padding:"12px",background:"#f8f7f5",borderRadius:8,border:"1px solid #e2ddd6"}}>
          <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{display:"flex",gap:16,alignItems:"center"}}>
                <label style={{fontSize:12,color:"#6b6560"}}>Type : <select value={q.type} onChange={e=>setQs(p=>p.map((x,j)=>j===i?{...x,type:e.target.value}:x))} style={{fontSize:12,border:"1px solid #e2ddd6",borderRadius:4,padding:"2px 6px"}}>
                  <option value="textarea">Texte libre</option><option value="mood">Humeur (smileys)</option><option value="presence">Présence</option>
                </select></label>
